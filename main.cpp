...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR lpCmdLine,
  ...
  if (!RegisterClassEx(&wClass)) {
    ...
    MessageBox(NULL, "WinGL Class creation failed\r\n", "WinGL Class Failed",
               MB_ICONERROR);
  }
    ...
    /*switch(LOWORD(wParam))
{
  ...
  case WM_LBUTTONUP: {
    ...
    if (mybutton3.LUp(mx, my) == 1) {
      if (MessageBox(hWnd, "Do you want to exit?", "WWM info", MB_YESNO) ==
          IDYES) {
        exit(0);
      }
    }
...
void CreateGraphicsPanel(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  GraphicsOpengl =
      CreateWindowEx(0, "STATIC", "", WS_CHILD | WS_VISIBLE, 0, 0, 1024, 720,
                     hWnd, (HMENU)IDC_GRX_OPENGL, NULL, NULL);
  ...
  HFONT font1;
  base = glGenLists(96);
  font1 = CreateFont(16, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, ANSI_CHARSET,
                     OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY,
                     FF_DONTCARE | DEFAULT_PITCH, "Console");
  ...
  for (uint i = 0; i < 5; i++)
    mytabs1.AddTabControl(3, &tab4statics[i]);
  for (uint i = 0; i < 5; i++)
    mytabs1.AddTabControl(3, &tab4wheels[i]);
  ...
  for (uint i = 0; i < 5; i++)
    tab4statics[i].font = base;
  for (uint i = 0; i < 5; i++)
    tab4wheels[i].font = base;
}
